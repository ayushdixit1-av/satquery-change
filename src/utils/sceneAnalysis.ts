  ...
  try {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        } else if (r > g && chroma < 26 && l > 80 && l < 215) {
          label = 'Bare';
        } else if (sat < 0.08 && l >= 40 && l <= 225) {
          label = 'Built-up';
          urbanPixels++;
        } else {
          label = 'Built-up';
        ...
        // Edge/texture: luminance jump to the right neighbour
        if (x < width - 1) {
    ...
    parts.push(
      `Overall brightness ${(brightness * 100).toFixed(0)}%, structural detail ${(detail * 100).toFixed(0)}%.`,
    );
